...
class Player(GameSprite):
    def init(self, player_image, player_x, player_y, size_x, size_y, player_x_speed, player_y_speed):
        super().init(player_image, player_x, player_y, size_x, size_y)
    ...
    def load_animation(self):
        ...
        frame_height = 24
        self.frames_right = [
            transform.scale(sheet_right.subsurface((i * frame_width, 0, frame_width, frame_height)), (64, 64))
            for i in range(4)
        ]
        # Для left можна зробити дзеркальні кадри:
        self.frames_left = [transform.flip(f, True, False) for f in self.frames_right]
...
class Enemy_h(GameSprite):
    side = "left"
    def init(self, player_image, player_x, player_y, size_x, size_y, player_speed, x1, x2):
...
class Enemy_v(GameSprite):
    side = "up"
    def init(self, player_image, player_x, player_y, size_x, size_y, player_speed, y1, y2):
        GameSprite.init(self, player_image, player_x, player_y, size_x, size_y)
        self.speed = player_speed
        self.y1 =y1
        self.y2 =y2
    ...
    if a == "menu":
        for e in event.get():
            if e.type == QUIT:
                ...
                if start_but.rect.collidepoint(e.pos):
                    ...
                    a = "lvl1"
                    mixer.music.load("game/sounds/sounds/Death Is Just Another Path.mp")
                    mixer.music.play(-1)
        ...
        if not finish:
            ...
            if sprite.spritecollide(hero, monsters, False):
                ...
                lose.play()
                mixer.music.stop()
                img = image.load('game/images/game_over.png')
